...
int execute(std::ifstream &file,bool write_result_to_file,int argc, char **argv,char *algorithm,const char * start_vertex,char * filename, const char * result_filename, bool measure_time) {
            ...
            if (computing_path_possible(graph)) {
                ...
                if (paths.empty()) {
                    ...
                    if (write_result_to_file) {
                        write_results_to_file(paths, start_vertex, result_filename, "Dijkstra",measure_time,measured_time.count());
                    }
...
int main(int argc, char **argv) {
    ...
    if (filename)
    {
        ...
        if(file) {
            if (start_vertex) {
                return execute(file, write_result_to_file,argc,argv,algorithm,start_vertex,filename,result_filename, measure_time);
            } else {
                // Musime to provest pro kazdy vertex
                return execute(file, write_result_to_file,argc,argv,algorithm,start_vertex,filename,result_filename, measure_time);
            }
        } else {
